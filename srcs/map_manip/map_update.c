...
/*   Updated: 2024/11/06 16:06:59 by gcros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	break_wall(t_map_p map, t_cub_p cub)
{
	...
	rcs = (t_rc_sett){.angle = cub->player.angle, .start = cub->player.pos,
		.attributes = {cf_breakable | cf_cast}, .max = 1};
	rc = rc_throw(map, rcs);
	if (rc.dist > 1.)
		return ;
...
static void	update_door(t_map_p map, t_cub_p cub)
{
	...
	rcs = (t_rc_sett){.angle = cub->player.angle, .start = cub->player.pos,
		.attributes = {cf_canclose | cf_canopen | cf_cast}, .max = 1};
	rc = rc_throw(map, rcs);
	if (rc.dist > 1. || rc.dist < (cub->player.size * .5))
		return ;
	cf = map_cell_setting(map_get_cell(map, floor(rc.pos.x), floor(rc.pos.y)),
